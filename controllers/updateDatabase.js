...
const getCurrencyList = "https://www.lb.lt/webservices/FxRates/FxRates.asmx/getCurrencyList";
const getCurrentFxRates = "https://www.lb.lt/webservices/FxRates/FxRates.asmx/getCurrentFxRates?tp=EU";
...
updateDatabase = asyncHandler(async (err, result) => {
    result.CcyTbl.CcyNtry.map(asyncHandler(async (elem) => {
        ...
        const currency = await Currency.findOneAndUpdate({ name: elem.Ccy[0] }, fieldsToUpdate, {
            new: true,
            runValidators: true
...
updateRates = asyncHandler(async (err, result) => {
    result.FxRates.FxRate.map(asyncHandler(async (elem) => {
        const fieldsToUpdate = {
            factor: elem.CcyAmt[1].Amt[0]
        };

        //if currency exists, update
        const currencyRate = await CurrencyRate.findOneAndUpdate({ currency: elem.CcyAmt[1].Ccy[0] }, fieldsToUpdate, {
            new: true,
            runValidators: true
